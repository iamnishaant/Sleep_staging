...
def create_binary_dataloaders(
    ...
    seed: int = 42,
    channel_names: Optional[Sequence[str]] = None,
    target_sample_rate: float = 128.0,
    normalization: str = "zscore",
) -> Tuple[DataLoader, DataLoader, Optional[DataLoader], dict]:
    """
    Create dataloaders for binary disease classification with stratified splits.

    Args:
        csv_path: Path to CSV with 'path' column and disease columns.
        disease_column: Name of disease column to use for binary classification.
        Other args: analogous to create_cfs_dataloaders.

    Returns:
        train_loader, val_loader, test_loader, stats
    """
    # Load and filter dataframe (reuse existing logic)
    df = pd.read_csv(csv_path)
    if "path" not in df.columns:
    ...
    # Build binary label dataframe (path + single disease column)
    binary_df = df[["path", disease_column]].copy()
    binary_df.columns = ["path", "label"]

    # Clean labels: map only 0/1 to valid, drop -2/NaN as bad values
    binary_df["label_clean"] = binary_df["label"].map({0: 0, 1: 1})
    valid_mask = binary_df["label_clean"].notna()  # Drops NaN and unmapped (-2)
    invalid_count = len(binary_df) - valid_mask.sum()
    if invalid_count > 0:
        print_warning(f"Dropping {invalid_count} invalid label samples (-2/NaN/etc.)")

    binary_df = binary_df.loc[valid_mask].reset_index(drop=True)
    binary_df["label"] = binary_df["label_clean"]  # Use cleaned labels
    binary_df = binary_df[["path", "label"]]  # Drop temp column

    total = len(binary_df)
    if total == 0:
        raise ValueError("No valid 0/1 samples after cleaning.")
    
    pos_ratio = (binary_df["label"] == 1).mean()
    print_key_value("Positive class ratio (0/1)", f"{pos_ratio:.3f}")

    # Stratified split: first test, then val from train, preserving 0/1 ratio
    # Note: sklearn handles ratio/absolute via float/int logic
    test_size = _resolve_split(test_split, total)
    temp_size = _resolve_split(val_split, total - test_size)
    
    # Seed sklearn RNG consistently
    rng = np.random.default_rng(seed)
    ...
    # Val split from remaining (stratified)
    train_idx, val_idx = train_test_split(
        train_temp_idx,
        test_size=temp_size,
        stratify=binary_df["label"].iloc[train_temp_idx],
        random_state=seed,
    )
    ...
    train_dataset = CFSAilmentDataset(indices=train_idx, **dataset_kwargs)
    val_dataset = CFSAilmentDataset(indices=val_idx, **dataset_kwargs)
    test_dataset = (
        CFSAilmentDataset(indices=test_idx, **dataset_kwargs) if test_size > 0 else None
    )
    ...
    stats = {
        ...
        "val": len(val_dataset),
        "test": len(test_dataset) if test_dataset is not None else 0,
        "num_labels": train_dataset.num_labels,  # should be 1
        "disease": disease_column,
    }

    print_info("Loaded CFS Visit 5 binary dataset from CSV")
    print_key_value("Disease column", disease_column)
    print_key_value("Total EDF files", stats["total"])
    print_key_value("Train samples", stats["train"])
    print_key_value("Validation samples", stats["val"])
    print_key_value("Test samples", stats["test"])
    ...
    # Verify stratification post-split
    train_pos = (binary_df["label"].iloc[train_idx] == 1).mean()
    val_pos = (binary_df["label"].iloc[val_idx] == 1).mean()
    print_key_value("Train pos ratio", f"{train_pos:.3f}")
    print_key_value("Val pos ratio", f"{val_pos:.3f}")
